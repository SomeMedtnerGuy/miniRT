...
/*   Updated: 2024/09/10 16:51:26 by joamonte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_cylinder(char **line, t_root *r)
{
	...
	if(line[5])
		clean_exit(r, 1);
	data[0] = ft_atotup(line[1]);
	if(!is_coordinates(data[0]))
		clean_exit(r, 1);
	data[1] = ft_atotup(line[2]);
	if(!is_vector(data[1]))
		clean_exit(r, 1);
	if(!ft_isstr_float(line[2]))
		clean_exit(r, 1);
